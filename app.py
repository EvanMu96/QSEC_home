...
import orm
from MyPy import add_routes, add_static
# Setting jinja2 when start the app
...
async def logger_factory(app, handler):
    async def parse_data(request):
        if request.method == 'POST':
            ...
            elif request.content_type.startswith('application/x-www-form-encoded'):
                request.__data__ = await request.post()
                logging.info('reques form: %s' % str(request.__data__))
